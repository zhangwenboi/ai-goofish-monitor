...
async def scrape_xianyu(task_config: dict, debug_limit: int = 0):
    ...
    if decision_mode not in {"ai", "keyword", "monitor"}:
        decision_mode = "ai"
                ...
                for page_num in range(1, max_pages + 1):
                    ...
                    for i, item_data in enumerate(basic_items, 1):
                        ...
                        if decision_mode == "monitor":
                            ...
                            item_id = str(item_data.get("商品ID") or unique_key)
                            is_new = unique_key not in processed_links

                            if is_new:
                                notify_reason = "新上架商品"
                            else:
                                last_price = get_last_known_price(keyword, item_id)
                                if last_price is not None and current_price is not None and current_price < last_price:
                                    notify_reason = f"价格下降: ¥{last_price} → ¥{current_price}"
                                else:
                                    continue

                            log_time(
                                f"[监控模式] [{notify_reason}] {item_data['商品标题'][:30]}..."
                            )
                            monitor_record = {
                                ...
                                "分析结果": {
                                    ...
                                    "analysis_source": "monitor",
                                },
                            }
                            save_to_jsonl(monitor_record, result_filename)
                            await send_ntfy_notification(item_data, notify_reason)
